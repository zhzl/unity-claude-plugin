...
- Optional model and color customization
...
Which model the agent should use. If omitted, Claude Code inherits the parent model.
...
**Default:** If omitted, Claude Code does not apply an agent-specific tool allowlist.

**Best practice:** Specify the minimum tools needed (principle of least privilege). Only omit `tools` when broad tool access is explicitly required.
...
### Fields not supported for plugin-shipped agents

Plugin-shipped agents should not use these project/user subagent fields:

- `permissionMode`
- `mcpServers`
- `hooks`

Claude Code ignores these fields for plugin-shipped agents. Keep plugin agent permissions and automation in supported locations such as tool restrictions (`tools`/`disallowedTools`) and plugin `hooks/hooks.json`.
...
| `disable-model-invocation` | Block programmatic Skill tool usage    | Agents use the Agent tool, not Skill tool               |
...
3. Select model if needed (omitted defaults to inherited model)
4. Choose color for visual identification if desired
5. Define the minimum tools needed
...
**Resuming agents:** Each Agent invocation creates a fresh agent. To continue with full prior context, ask Claude to "resume that agent" — it will restore the previous transcript.

**Restricting spawnable agents:** Use `Agent(agent_type1, agent_type2)` syntax in settings.json allow rules to control which agent types can be spawned. Omitting `Agent` entirely prevents subagent spawning.
...
Teams provide shared task lists, inter-agent messaging, and parallel execution. For coordination-focused team leads, use tool restrictions plus explicit prompt guidance to keep the lead focused on delegation and review. `permissionMode` is ignored for plugin-shipped agents.
...
- **`references/advanced-agent-fields.md`** - Detailed docs for supported plugin agent fields such as maxTurns and memory, plus clearly labeled project/user subagent-only fields
- **`references/permission-modes-rules.md`** - Permission mode details for project/user subagents; these modes are ignored for plugin-shipped agents
...
These scripts assume a shell environment with `bash` 3.2+ and standard POSIX userland utilities (`grep`, `sed`, `awk`, `cut`, `tr`, `head`, `tail`, `basename`, `mktemp`). Check optional tool availability when debugging script failures.

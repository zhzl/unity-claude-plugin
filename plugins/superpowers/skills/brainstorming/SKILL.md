...
3. **主动挑战扫描** — 在提出方案或展示设计前，完整扫描会影响目标、范围、设计边界、验证方式或后续实现计划的隐藏问题
4. **逐条确认关键问题** — 展示完整清单后，先确认“必须先确认”的最高影响问题；一次只推进一个
5. **提出 2-3 种方案** — 只在关键问题已确认或明确记录为设计假设后进行，附带权衡分析和你的推荐
6. **展示设计** — 按复杂度分节展示，每节展示后获得用户批准
7. **编写设计文档** — 保存到 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` 并 commit
8. **规格自检** — 快速内联检查占位符、矛盾、模糊性、范围（详见下方）
9. **用户审查书面规格** — 在继续之前请用户审查规格文件
...
在提出方案或展示设计前，必须执行主动挑战扫描；不要只顺着用户已提出的问题推进。

主动挑战扫描不设固定上限。如果存在更多会影响目标、范围、设计边界、验证方式或后续实现计划的高影响问题，必须继续列出。不要为了简短省略关键问题，也不要为了凑数列低价值问题。

扫描结果按优先级分组：

1. **必须先确认**
   - 不确认就会改变目标、范围、架构边界、验证方式、接口契约或实现计划的问题。
2. **建议确认**
   - 不确认也能继续，但后续可能返工、产生歧义或影响方案质量的问题。
3. **可作为设计假设**
   - 当前可以先采用默认假设，但必须写明假设、风险和后续验证点。

展示完整清单后，只展开“必须先确认”中的第一条，按逐条确认格式推进。用户确认后再进入下一条；不要一次要求用户回答整份清单。

高影响主动挑战问题不能只通过 `AskUserQuestion` 选择题呈现。必须先用 Markdown 展示完整说明、为什么重要、选项和推荐理由；`AskUserQuestion` 只能作为辅助收集选择，不能替代问题解释。
...
1. **问题是什么：**
   ...
   - **用户容易理解的说明：** 用具体场景说明这个问题影响什么。
   - **为什么重要：** 如果不确认，会导致什么错误设计、返工或边界漂移。
2. **需要用户确认什么：**
...
- 只有在“必须先确认”的关键问题已确认，或明确记录为设计假设后，才提出方案。
- 提出 2-3 种不同的方案及其权衡。
...
- 一旦你完成主动挑战扫描，并确认或记录所有“必须先确认”的问题，就展示设计

...
- 顺序检查：任务依赖、前后顺序与里程碑安排是否一致，是否出现倒置或缺少前置步骤。
...
- 执行性检查：每个任务是否具备可执行所需的明确输入、输出、落点与操作细节，避免只给抽象指令却无法直接开始或完成。
...
- 如果审查发现 plan 可能应拆分，只能建议拆分为多个 plans、回到上游 scope/roadmap 决策流程，或要求 plan 记录用户接受单一 plan 风险；不得自行重写 plan 或决定上游 phase split 方法。

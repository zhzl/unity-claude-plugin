...
PLUGIN_DEV_AUDIT_ROADMAP="$PLUGIN_ROOT/../../docs/superpowers/roadmaps/2026-05-17-plugin-dev-audit/ROADMAP.md"
...
assert_file_exists "$PLUGIN_DEV_AUDIT_ROADMAP" "Plugin-dev audit roadmap exists"
...
assert_file_contains "$SKILL" "| Phase | Status | Goal | Spec | Plan | Verification | Next |" "Phase Summary table format is locked in skill"
assert_file_contains "$SKILL" '必须读取 `references/roadmap-format.md` 并按其中模板套用结构' "new-roadmap draft must use the format reference"
assert_file_contains "$SKILL" "不要自造 section/table 结构" "Roadmap table formats cannot be invented"
...
assert_file_contains "$SKILL" "主动挑战扫描" "Roadmap must run active challenge scan"
assert_file_contains "$SKILL" "不设固定上限" "Roadmap challenge scan has no fixed upper limit"
assert_file_contains "$SKILL" "必须先确认" "Roadmap challenge scan prioritizes must-confirm issues"
assert_file_contains "$SKILL" "建议确认" "Roadmap challenge scan separates suggested confirmations"
assert_file_contains "$SKILL" "可作为 roadmap 假设" "Roadmap challenge scan can record roadmap assumptions"
assert_file_contains "$SKILL" "展示完整清单" "Roadmap challenge scan requires showing the full list"
assert_file_contains "$SKILL" "未确认挑战清单" "write-spec carries unresolved challenge list"
assert_file_contains "$SKILL" "已确认结论和未确认挑战清单" "Brainstorming handoff carries confirmed conclusions and unresolved challenges"
assert_file_contains "$SKILL" "为什么重要" "Roadmap clarification must explain why the issue matters"
assert_file_contains "$SKILL" "AskUserQuestion" "Roadmap high-impact questions mention AskUserQuestion limitation"
assert_file_contains "$SKILL" "不能替代问题解释" "AskUserQuestion cannot replace roadmap explanation"
assert_file_contains "$SKILL" "主动澄清格式" "Roadmap active clarification format section exists"
...
assert_file_contains "$SKILL" "确认问题" "Roadmap discussion must state the decision question before options"
assert_file_contains "$SKILL" "必须先列完全部选项" "Roadmap discussion must list all options before recommendation"
assert_file_contains "$SKILL" '不要在 `选项 A/B/C` 标题里写“推荐”' "Roadmap discussion must not put recommendation in option titles"
assert_file_contains "$SKILL" "不要在列完所有选项前插入推荐理由" "Roadmap discussion must not insert recommendation reason before all options"
assert_file_contains "$SKILL" '`推荐选项` 必须是独立字段' "Roadmap recommendation must be an independent field"
assert_file_contains "$SKILL" "请先回复 A / B / C" "Roadmap discussion must ask for A/B/C response"
...
echo ""
echo "Test 7: Created roadmaps follow roadmap-format tables..."
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Metadata" '- **Current Phase:** Phase 1 — 审计标准与文件矩阵' "Roadmap metadata includes current phase"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Metadata" '- **Next Action:** write-spec' "Roadmap metadata includes next action"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Decisions" '| Date | Decision | Reason |' "Roadmap decisions use reference table"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Current State" '- **Overall Status:** active' "Roadmap current state includes overall status"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Current State" '- **Current Phase Status:** needs-spec' "Roadmap current state includes phase status"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Current State" '- **Last Sync Evidence:** Initial roadmap' "Roadmap current state includes sync evidence"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Blockers" '| Blocker | Affects | Status | Resolution |' "Roadmap blockers use reference table"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Phase Summary" '| Phase | Status | Goal | Spec | Plan | Verification | Next |' "Roadmap Phase Summary uses reference table"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "### Phase 1 — 审计标准与文件矩阵" '**Goal:**' "Phase details include goal"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "### Phase 1 — 审计标准与文件矩阵" '**Depends on:**' "Phase details include dependency"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "### Phase 1 — 审计标准与文件矩阵" '**Next Manual Action:**' "Phase details include next manual action"
assert_section_contains "$PLUGIN_DEV_AUDIT_ROADMAP" "## Change Log" '| Date | Change | Evidence |' "Roadmap change log uses reference table"
